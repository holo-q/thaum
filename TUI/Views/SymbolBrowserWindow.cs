...
public class SymbolBrowserWindow : View {
	...
	private readonly ILogger<SymbolBrowserWindow> _log;
	...
	public SymbolBrowserWindow(
		...
		_log        = Logging.For<SymbolBrowserWindow>();
	...
	private void InitializeComponents() {
		...
		// Colors handled by Terminal.Gui v2 theming
		...
		_titleBar = new Label {
			...
			Width  = Dim.Fill(),
			Height = 1,
			// ColorScheme handled by Terminal.Gui v2
		};
	...
	private void LoadSymbols() {
		...
		_log.LogInformation("Loaded {Count} files with {SymbolCount} symbols",
			_state.CodeMap.FileCount,
			_state.CodeMap.Count);
	...
	private void SetupCommands() {
		...
		AddCommand(Command.PageUp, ctx => {
			NavigatePage(-1);
			return true;
		});
		AddCommand(Command.PageDown, ctx => {
			NavigatePage(1);
			return true;
		});
		...
		// Refresh
		AddCommand(Command.Refresh, ctx => {
			RefreshSymbols();
			return true;
		});
	...
	private bool NavigatePage(int direction) {
		// Calculate page size based on visible area (subtract title and status bars)
		int pageSize = Math.Max(1, Bounds.Height - 2);
		int newIndex = _state.SelectedIndex + (direction * pageSize);
		
		// Clamp to valid range
		newIndex = Math.Max(0, Math.Min(newIndex, _state.DisplayNodes.Count - 1));
		
		if (newIndex != _state.SelectedIndex) {
			_state.SelectedIndex = newIndex;
			_symbolTree.SetSelection(_state.SelectedIndex);
			_symbolTree.EnsureSelectionVisible();
			_statusBar.UpdateDisplay();
			return true;
		}
		return false;
	...
	private void OnApplicationResized(ResizeEventArgs e) {
		// Force redraw of all components after resize
		SetNeedsDraw();
		UpdateTitleBar();
		_symbolTree?.SetNeedsDraw();
		_statusBar?.SetNeedsDraw();
		_titleBar?.SetNeedsDraw();
	}
