...
public static class TriadRepairer {
	...
	static string BuildRepairPrompt(CodeSymbol symbol, string source, FunctionTriad triad, List<string> missing) {
		...
		sb.AppendLine("</sourceCode>");
		sb.AppendLine();
		if (!string.IsNullOrWhiteSpace(triad.Topology)) {
			sb.AppendLine("<TOPOLOGY>");
			sb.AppendLine(triad.Topology!);
			sb.AppendLine("</TOPOLOGY>");
		}
		if (!string.IsNullOrWhiteSpace(triad.Morphism)) {
			sb.AppendLine("<MORPHISM>");
			sb.AppendLine(triad.Morphism!);
			sb.AppendLine("</MORPHISM>");
		}
		if (!string.IsNullOrWhiteSpace(triad.Policy)) {
			sb.AppendLine("<POLICY>");
			sb.AppendLine(triad.Policy!);
			sb.AppendLine("</POLICY>");
		}
		if (!string.IsNullOrWhiteSpace(triad.Manifest)) {
			sb.AppendLine("<MANIFEST>");
			sb.AppendLine(triad.Manifest!);
			sb.AppendLine("</MANIFEST>");
		}
		sb.AppendLine();
