...
public class SymbolListView : FrameView
{
    ...
    {
        foreach (var symbol in symbols.OrderBy(s => s.StartPosition.Line))
        {
            // Add indentation for nested symbols
            var displaySymbol = indent > 0 ? 
                symbol with { Name = new string(' ', indent * 2) + symbol.Name } : 
                symbol;
    ...
    {
        var items = _symbols.Select(FormatSymbolItem).ToArray();
    ...
    {
        var icon = GetSymbolIcon(symbol.Kind);
        var status = GetSymbolStatus(symbol);
