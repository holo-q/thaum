	...
	private void LoadProject(string projectPath) {
		Task.Run(async () => {
			try {
				...
				bool started = await _languageServerManager.StartLanguageServerAsync(language, projectPath);
				if (!started) {
	...
	private void RefreshSymbols() {
		...
		Task.Run(async () => {
			try {
				string? language = DetectPrimaryLanguage(_currentProjectPath);
				if (language != null) {
					List<CodeSymbol> symbols = await _languageServerManager.GetWorkspaceSymbolsAsync(language, _currentProjectPath);
					Application.MainLoop.Invoke(() => {
