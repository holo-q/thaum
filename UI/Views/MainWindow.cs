...
public class MainWindow : Window
{
    ...
    {
        var dialog = new OpenDialog("Open Project", "Select project folder")
        {
        ...
        var projectPath = dialog.FilePaths.First();
    ...
    private void LoadProject(string projectPath)
    {
        Task.Run(async () =>
        {
            try
            {
                ...
                var language = DetectPrimaryLanguage(projectPath);
                if (language == null)
                {
                ...
                var started = await _lspManager.StartLanguageServerAsync(language, projectPath);
                if (!started)
                {
                    Application.MainLoop.Invoke(() => SetStatusText($"Failed to start {language} language server"));
                    return;
                }

                // Load project structure
                var projectFiles = Directory.GetFiles(projectPath, "*.*", SearchOption.AllDirectories)
    ...
    private void StartSummarization()
    {
        ...
        Task.Run(async () =>
        {
            try
            {
                ...
                var language = DetectPrimaryLanguage(_currentProjectPath);
                if (language == null)
                {
                    Application.MainLoop.Invoke(() => SetStatusText("No supported language detected"));
                    return;
                ...
                var hierarchy = await _summarizationEngine.ProcessCodebaseAsync(_currentProjectPath, language);
    ...
    private void RefreshSymbols()
    {
        ...
        Task.Run(async () =>
        {
            try
            {
                ...
                if (language != null)
                {
                    var symbols = await _lspManager.GetWorkspaceSymbolsAsync(language, _currentProjectPath);
                    Application.MainLoop.Invoke(() =>
                    {
    ...
    private static string? DetectPrimaryLanguage(string projectPath)
    {
        ...
        var primaryExtension = extensionCounts
