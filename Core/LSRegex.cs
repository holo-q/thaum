...
public class LSRegex : ILanguageServer {
	private readonly ILogger<LSRegex> _logger;
	private readonly Dictionary<string, bool>        _runningServers = new();

	public LSRegex(ILogger<LSRegex> logger) {
		_logger = logger;
	}
	...
	public async Task<List<CodeSymbol>> GetWorkspaceSymbolsAsync(string language, string workspacePath) {
		...
		try {
			...
			foreach (string file in sourceFiles) {
				List<CodeSymbol> fileSymbols = await ExtractSymbolsFromFile(file, language);
	...
	public void Dispose() {
		foreach (string language in _runningServers.Keys.ToList()) {
