...
/// </summary>
public class ProperLspClientManager : ILspClientManager {
    private readonly ILogger<ProperLspClientManager> _logger;
    private readonly LspServerManager _serverManager;
    private readonly Dictionary<string, LspServerInstance> _servers = new();

    public ProperLspClientManager(ILogger<ProperLspClientManager> logger) {
        _logger = logger;
        _serverManager = new LspServerManager(
            LoggerFactory.Create(b => b.AddConsole().SetMinimumLevel(LogLevel.Debug)).CreateLogger<LspServerManager>(),
    ...
    public async Task<bool> StartLanguageServerAsync(string language, string workspacePath) {
        ...
        try {
            ...
            var executablePath = await _serverManager.GetLspServerPathAsync(language);
            ...
            var serverConfig = GetServerConfiguration(language, executablePath);
            if (serverConfig == null) {
                _logger.LogWarning("No LSP server configuration found for language: {Language}", language);
                return false;
            }

            var instance = new LspServerInstance(language, workspacePath, serverConfig, _logger);
    ...
    public async Task<bool> StopLanguageServerAsync(string language) {
        if (!_servers.TryGetValue(language, out var instance)) {
            return true;
        }
    ...
    public async Task<List<CodeSymbol>> GetWorkspaceSymbolsAsync(string language, string workspacePath) {
        if (!_servers.TryGetValue(language, out var instance) || !instance.IsRunning) {
    ...
    public bool IsLanguageServerRunning(string language) {
        return _servers.TryGetValue(language, out var instance) && instance.IsRunning;
    }
    ...
    public async IAsyncEnumerable<SymbolChange> WatchWorkspaceChanges(string language, string workspacePath) {
        ...
        await foreach (var change in instance.WatchWorkspaceChanges()) {
            yield return change;
        }
    }

    public void Dispose() {
        var stopTasks = _servers.Values.Select(instance => instance.StopAsync()).ToArray();
    ...
    private static string FindOmniSharpExecutable() {
        ...
        var possibleNames = new[] { "omnisharp", "OmniSharp", "OmniSharp.exe" };
        
        foreach (var name in possibleNames) {
            // Check if executable exists in PATH
            try {
                var startInfo = new ProcessStartInfo(name, "--version") {
...
internal class LspServerInstance : IDisposable {
    ...
    public async Task<bool> StartAsync() {
        try {
            ...
            var success = await InitializeLspConnection();
            if (success) {
                _initialized = true;
        ...
        try {
            ...
            if (headers.TryGetValue("Content-Length", out var lengthStr) && 
                ...
                while (totalRead < length) {
                    var read = await _reader.ReadAsync(buffer, totalRead, length - totalRead);
                    if (read == 0) {
