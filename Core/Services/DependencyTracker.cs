...
public class DependencyTracker : IDependencyTracker
{
    ...
    public async Task BuildDependencyGraphAsync(string projectPath, string language)
    {
        try
        {
            ...
            var analyzer = CreateAnalyzer(language);

            foreach (var file in sourceFiles)
            {
                var dependencies = await analyzer.GetDependenciesAsync(file);
    ...
    public async Task UpdateDependencyAsync(string filePath, List<string> dependencies)
    {
        try
        {
            var projectPath = FindProjectPath(filePath);
            if (projectPath != null && _projectGraphs.TryGetValue(projectPath, out var graph))
            {
    ...
    public async Task<List<string>> GetDependentsAsync(string filePath)
    {
        try
        {
            ...
            var dependents = graph.GetDependents(filePath);
            await Task.CompletedTask;
    ...
    public async Task<List<string>> GetDependenciesAsync(string filePath)
    {
        try
        {
            ...
            var dependencies = graph.GetDependencies(filePath);
            await Task.CompletedTask;
    ...
    private IDependencyAnalyzer CreateAnalyzer(string language)
    {
        return language.ToLowerInvariant() switch
        {
            ...
            "csharp" => new CSharpDependencyAnalyzer(),
...
internal class ProjectDependencyGraph
{
    ...
    public void SetDependencies(string filePath, List<string> dependencies)
    {
        ...
        var newDeps = dependencies.ToHashSet();
        ...
        foreach (var dep in dependencies)
        {
            _dependents.AddOrUpdate(dep, 
                ...
                (_, existing) => { existing.Add(filePath); return existing; });
        }
    }

    public List<string> GetDependencies(string filePath)
    {
        return _dependencies.TryGetValue(filePath, out var deps) 
            ? deps.ToList() 
            : new List<string>();
    }
    ...
    {
        return _dependents.TryGetValue(filePath, out var deps) 
            ? deps.ToList() 
            : new List<string>();
    }
    ...
    public async Task<List<string>> GetDependenciesAsync(string filePath)
    {
        ...
        try
        {
            ...
            foreach (Match match in matches)
            {
                ...
                var items = importedItems.Split(',').Select(i => i.Trim().Split(' ')[0]);
...
internal class JavaScriptDependencyAnalyzer : IDependencyAnalyzer
{
    ...
    public async Task<List<string>> GetDependenciesAsync(string filePath)
    {
        ...
        try
        {
            ...
            foreach (Match match in matches)
            {
                var moduleName = match.Groups[1].Value.IsNullOrEmpty() ? match.Groups[2].Value : match.Groups[1].Value;
...
internal class RustDependencyAnalyzer : IDependencyAnalyzer
{
    ...
    public async Task<List<string>> GetDependenciesAsync(string filePath)
    {
        ...
        {
            var content = await File.ReadAllTextAsync(filePath);
            var matches = UseRegex.Matches(content);
            
            foreach (Match match in matches)
            {
                var useStatement = match.Groups[1].Value.Trim();
                var rootModule = useStatement.Split(new[] { "::", "." }, StringSplitOptions.RemoveEmptyEntries)[0];
