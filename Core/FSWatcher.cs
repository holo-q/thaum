...
public class FSWatcher : IDisposable {
	...
	public async Task StartWatchingAsync(string projectPath, string language) {
		...
		try {
			ProjectWatcher watcher = new ProjectWatcher(projectPath, language, _crawler, _dependencyTracker, _logger);
	...
	public async Task StopWatchingAsync(string projectPath) {
		if (_watchers.TryRemove(projectPath, out ProjectWatcher? watcher)) {
	...
	public async IAsyncEnumerable<FileChangeEvent> GetChangeEventsAsync(string projectPath) {
		if (!_watchers.TryGetValue(projectPath, out ProjectWatcher? watcher)) {
			yield break;
		}

		await foreach (FileChangeEvent changeEvent in watcher.GetChangesAsync()) {
	...
	public async Task<List<CodeSymbol>> GetAffectedSymbolsAsync(string filePath, ChangeType changeType) {
		try {
			string? projectPath = FindProjectPathForFile(filePath);
			if (projectPath == null || !_watchers.TryGetValue(projectPath, out ProjectWatcher? watcher)) {
	...
	public async Task<List<string>> GetDependentFilesAsync(string filePath, string language) {
		try {
			List<string> dependents = await _dependencyTracker.GetDependentsAsync(filePath);
			return dependents;
		} catch (Exception ex) {
	...
	public void Dispose() {
		foreach ((string _, ProjectWatcher watcher) in _watchers) {
...
internal class ProjectWatcher : IDisposable {
	...
	private void EnqueueChange(string filePath, ChangeType changeType) {
		Task.Run(async () => {
			try {
				List<CodeSymbol> affectedSymbols = await GetAffectedSymbolsAsync(filePath, changeType);
