...
public class LSPManager {
	...
	public LSPManager(ILogger<LSPManager> logger, HttpClient? httpClient = null, ILspDownloadProgress? progressReporter = null) {
		_logger           = logger;
	...
	public async Task<string?> GetLspServerPathAsync(string language) {
		...
		string serverDir      = Path.Combine(_cacheDirectory, language);
	...
	private string? FindExecutableInDirectory(string directory, string expectedName) {
		...
		string[] possibleNames = new[] { "OmniSharp", "omnisharp", "OmniSharp.exe", "run" };
	...
	public async Task CleanupOldServersAsync() {
		try {
			string[] dirs = Directory.GetDirectories(_cacheDirectory);
